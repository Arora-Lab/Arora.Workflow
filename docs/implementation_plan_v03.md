# Arora Labs — Arora.Workflow: Implementation Plan
# Version 03
...
## What Phase 1 Actually Is (Documentation)
...
## Phase 2: Core Engine Execution (Current Phase)

Phase 2 shifts focus to building the functional core of the SDK. Now that the basic test harness and DI scaffolding are working, we must build the core engine execution logic, approval entities, and verify the `WorkflowService` contracts.

### Phase 2 Implementation Tasks

1.  **WorkflowService Completion**: Write remaining tests for `WorkflowService` (`CancelAsync`, `GetInstanceAsync`, `GetHistoryAsync`) to verify correct interactions and domain event publishing before the engine gets complex.
2.  **Approval System Implementation**: Implement the missing `Approval` aggregate/entity, `IApprovalRepository`, and wire them into `ApprovalService`. Write `ApprovalServiceTests` to verify idempotent approvals and rejections.
3.  **WorkflowEngine Implementation**: Replace `WorkflowEngineStub` with the real `WorkflowEngine`. This involves parsing the `DefinitionJson`, executing steps (synchronous for now), evaluating transitions, and properly updating the `WorkflowInstance` state (e.g., stopping at `PendingApproval` states).
...
> [!WARNING]
> **Phase 2: WorkflowEngine Parsing Strategy**
> Currently, `WorkflowDefinition.DefinitionJson` stores the steps. To implement `WorkflowEngine`, we need a structured deserialization model (e.g., `WorkflowGraph` or `StepDefinition`) to represent the flowchart/state machine. For this phase, should we implement a full JSON deserializer for `DefinitionJson` right now, or should we use a strongly-typed stub graph for the engine just to prove state transitions work before we build the fluent builder API that generates the JSON?

> [!WARNING]
> **Phase 2: Approval Aggregate vs Entity**
> According to DDD, `WorkflowInstance` is the aggregate root. However, approvals are queried independently (e.g., "give me all my pending approvals"). We will create an `Approval` entity that acts as its own aggregate root for querying purposes, but is conceptually linked to `WorkflowInstance`.

---

## Proposed Changes (Phase 1 Documentation)
...
- **Aggregate roots**: `WorkflowDefinition`, `WorkflowInstance`, `Approval`
- **Entities**: `WorkflowStep`, `StepResult`, `EscalationPolicy`
